...
@pytest.mark.parametrize(
    "cgo_disable, force_gomod_tidy",
    (
        pytest.param(False, False, id="cgo_disabled_and_dont_force_tidy"),
        pytest.param(True, False, id="cgo_enabled_and_dont_force_tidy"),
        pytest.param(False, True, id="cgo_disabled_and_force_tidy"),
        pytest.param(True, True, id="cgo_enabled_and_force_tidy"),
    ),
)
@mock.patch("cachi2.core.package_managers.gomod.Go.release", new_callable=mock.PropertyMock)
@mock.patch("cachi2.core.package_managers.gomod._get_gomod_version")
...
@mock.patch("cachi2.core.package_managers.gomod._validate_local_replacements")
@mock.patch("subprocess.run")
def test_resolve_gomod_with_workspaces(
    mock_run: mock.Mock,
    mock_validate_local_replacements: mock.Mock,
    mock_version_resolver: mock.Mock,
    mock_get_gomod_version: mock.Mock,
    ...
    run_side_effects.append(
        proc_mock(
            "go mod download -json",
            returncode=0,
            stdout=get_mocked_data(data_dir, "workspaces/go_mod_download.json"),
        )
    )
    if force_gomod_tidy:
        run_side_effects.append(proc_mock("go mod tidy", returncode=0, stdout=None))
    run_side_effects.append(
        proc_mock(
            "go list -e -mod readonly -m",
            returncode=0,
            stdout=get_mocked_data(data_dir, "workspaces/go_list_modules.json").replace(
                "{repo_dir}", str(module_dir)
            ),
        )
    )
    run_side_effects.append(
        proc_mock(
            "go list -e -mod readonly -deps -json all",
            returncode=0,
            stdout=get_mocked_data(data_dir, "workspaces/go_list_deps_all.json"),
        )
    )
    run_side_effects.append(
        proc_mock(
            "go list -e -mod readonly -deps -json ./...",
            returncode=0,
            stdout=get_mocked_data(data_dir, "workspaces/go_list_deps_threedot.json"),
        )
    )
    mock_run.side_effect = run_side_effects
    ...
    expect_result = _parse_mocked_data(data_dir, "expected-results/resolve_gomod_workspaces.json")
...
def test_get_go_work_path(
    ...
    go_work_root = _get_go_work_path(repo_root)
...
def test_get_go_work_path_when_go_work_is_outside_of_repo(
    ...
    error_message = (
        f"Joining path '/a/random/path' to '{tmp_path}': target is outside '{tmp_path}'"
    )
...
def test_create_modules_from_parsed_data(
    ...
    if has_workspaces:
        go_work_root = RootedPath(tmp_path).join_within_root("workspace_dir")
